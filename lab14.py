...
def heapify_down(heap: MinHeap, index: int) -> MinHeap:
    ...
    if left < current_size and heap_data[left] < heap_data[least]:
        least = left

    if right < current_size and heap_data[right] < heap_data[least]:
        least = right

    if least != index:
        new_heap_data = heap_data[:]
        new_heap_data[index], new_heap_data[least] = new_heap_data[least], new_heap_data[index]
        return heapify_down(MinHeap(new_heap_data), least)
    else:
        return heap  # No swap needed, return the heap as is
